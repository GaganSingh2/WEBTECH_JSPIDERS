//Perfom Some Operation using CallBackHell-------------
...
//To Optimize the Promise Chaining using Async And Await--------------
function saveToDB(data){
    return new Promise((resolve, reject)=>{
        let internetSpeed = Math.floor(Math.random() * 10) + 1
        if(internetSpeed > 4){
...
async function user() {
    try{
        // Multiple await calls (replacement for Promise Chaining)
        let req1 = await saveToDB("Dishuuu")
        console.log(req1);

        let req2 = await saveToDB("Gagan")
        console.log(req2);

        let req3 = await saveToDB("Karan")
        console.log(req3);

        let req4 = await saveToDB("Vishal")
        console.log(req4);
    }
    catch(error){
        console.log(error);
